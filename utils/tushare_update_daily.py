...
def write_to_mysql_with_update(df_data):
    ...
    INSERT INTO cn_stock_daily (
        ...
        price_pre_close, amt_chg, pct_chg, vol, amount, update_date
...
def get_daily_data_by_day(start_date, end_date):
    ...
    for day_count in range(total_days):
        ...
        if not df.empty:
            ...
            print(
                f"           ✅ 写入完成：当日总条目 {day_record_count} 条，更新 {day_updated} 条，新增 {day_total - day_updated} 条", flush=True)
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"参数读取错误: {e}, 使用默认日期", flush=True)
        start_date = today
        end_date = today

    # 输出任务信息
    print(f"开始按天获取数据，日期范围: {start_date} 到 {end_date}", flush=True)
    print("=" * 50, flush=True)
    ...
    try:
        ...
        if has_data:
            ...
            col_width = 14
            print(f"{'年份':<10} {'累计写入':>{col_width}} {'累计更新':>{col_width}} {'新增':>{col_width}}", flush=True)
            print("-" * 60, flush=True)
            ...
            for year in sorted(year_stats.keys()):
                ...
                print(f"{year:<10}{write_count}{update_count}{new_count}", flush=True)
            print("-" * 60, flush=True)
        ...
        if has_data:
            ...
            print(f"总记录数: {total_record:,}", flush=True)
            result_msg = f"累计写入 {total_write:,} 条，累计更新 {total_update:,} 条，新增 {total_write - total_update:,} 条"
            print(f"📊 数据库写入汇总：{result_msg}", flush=True)
            
            # 记录成功日志
            try:
                log_task_execution("日K线抽取", "SUCCESS", f"日期范围: {start_date} - {end_date}, {result_msg}")
            except Exception:
                pass
        else:
            print("没有获取到任何数据", flush=True)
            try:
                log_task_execution("日K线抽取", "SUCCESS", f"日期范围: {start_date} - {end_date}, 本次执行没有获取到数据")
            except Exception:
                pass
                
    except Exception as e:
        print(f"❌ 任务执行出错: {e}", flush=True)
        try:
